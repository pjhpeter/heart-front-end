...
export default class UserOperation extends Vue {
  ...
  private showChangePasswordModal(): void {
    ...
    document.getElementById("app")!.click();
    // 判断修改密码模态框是否已经打开
    if (this.changePasswordModalId) {
      const modal: any = Commons.findModalById(this.changePasswordModalId);
      if (modal) {
    ...
    const modalInfo: ModalInfo = {
      url: "/heart/home/menu/changePasswordModal.vue",
      backgroundColor: Commons.getIconColor(),
      title: "修改密码",
      width: 250,
      className: "changePasswordM",
      resizable: false,
      enabledFuscreen: false,
      footerHide: false,
      onOk: function(component: any) {
        component.submit();
      }
    };
